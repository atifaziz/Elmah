...
namespace Elmah.Tests
{
    ...
    public class ErrorLogModuleTests
    {
        [Fact]
        ...
        sealed class TestErrorLogModule : ErrorLogModule
        {
            public Action<Exception, HttpContextBase> LogExceptionOverride { private get; set; }

            public TestErrorLogModule(ErrorLog log = null)
            {
                Log = log ?? new TestErrorLog();
            }
            ...
            protected override ErrorLog GetErrorLog(HttpContextBase context)
            {
                return Log ?? base.GetErrorLog(context);
            }
        }
    }
}
